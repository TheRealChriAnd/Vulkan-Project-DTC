...
// buffer inputs
layout(binding=BINDING_COLOR) buffer inColor { vec4 color_in[]; };
layout(location=BINDING_COLOR) out vec4 fragColor;

layout(binding=BINDING_UV) buffer inTexCoord { vec2 uv_in[]; };
layout(location=BINDING_UV) out vec2 fragTexCoord;

layout(binding=BINDING_POS) buffer pos 
{
	vec4 position_in[];
};
...
void main() {
	gl_Position = ubo.proj * ubo.view * ubo.model * position_in[gl_VertexIndex];
